// Session store: state shape + the full event-application logic
// (applyEvent → applyOne) and actions. A PURE .ts module on purpose —
// no JSX — so bun unit tests can value-import createSessionStore and
// drive the real event path (tests/store_notices.test.ts). The Solid
// context/provider wrapper lives in ./store.tsx.
import { batch, onCleanup } from "solid-js"
import { createStore } from "solid-js/store"
import { isOutOfBandNotice } from "../lib/notices"
...
/** A fresh State per store instance — createStore mutates its backing
 * object, so sharing one literal would bleed state between instances
 * (only visible once tests create more than one store). */
...
/** Build the full session store (state + actions), independent of any
 * JSX/provider. Exported so bun unit tests can drive the REAL event path
 * (applyEvent → applyOne) — e.g. the commandInFlight gate on
 * system.notice is load-bearing and must be directly testable. */
export function createSessionStore() {
  const [state, setState] = createStore<State>(freshState())
  ...
  const applyOne = (event: EventEnvelope) => {
    ...
    switch (event.event_type) {
      ...
      case "system.notice": {
        ...
        // scrollable transcript. OUT-OF-BAND notices are exempt: the async
        // update check emits its notice exactly ONCE per session — dropping
        // it because some unrelated command happened to be in flight would
        // lose it forever.
        if (state.commandInFlight && !isOutOfBandNotice(event.payload)) {
          return
        }
