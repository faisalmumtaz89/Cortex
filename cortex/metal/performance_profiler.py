...
class PerformanceProfiler:
    ...
    def estimate_flops(
        ...
        flops_map: Dict[str, Callable[[Tuple[int, ...]], float]] = {
            "matmul": lambda shape: float(2 * shape[0] * shape[1] * shape[2]) if len(shape) >= 3 else 0.0,
            "attention": lambda shape: float(4 * shape[0] * shape[1] * shape[1] * shape[2]),
            "layernorm": lambda shape: float(3 * shape[0] * shape[1]),
            "gelu": lambda shape: float(10 * shape[0] * shape[1]),
            "softmax": lambda shape: float(3 * shape[0] * shape[1]),
        }
