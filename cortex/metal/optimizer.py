...
class MetalOptimizer:
    ...
    def __init__(self, config: Optional[OptimizationConfig] = None):
        ...
        self._backend_optimizer: Optional[Any] = None
    ...
    def optimize_model(
        ...
        optimizations_applied = cast(list[str], optimization_info["optimizations_applied"])
        ...
        if self.backend == Backend.MLX:
            ...
            optimizations_applied.append("none")
        ...
        if self.config.use_quantization:
            ...
            optimizations_applied.append(f"int{self.config.quantization_bits}_quantization")
    ...
    def _apply_quantization(self, model: Any, bits: int) -> Any:
        ...
        if self.backend == Backend.MLX:
            ...
            backend_optimizer = self._backend_optimizer
            if backend_optimizer is not None and hasattr(backend_optimizer, "quantize_model"):
                return backend_optimizer.quantize_model(model, bits)
            ...
            try:
                ...
                quantizer = DynamicQuantizer(
                    QuantizationConfig(
                        mode=QuantizationMode.INT8 if bits == 8 else QuantizationMode.INT4,
                    )
                )
    ...
    def profile_inference(
        ...
        if self.backend == Backend.MLX and self._backend_optimizer:
            return cast(
                Dict[str, Any],
                self._backend_optimizer.profile_model(model, input_shape, num_iterations),
            )
    ...
    def cleanup(self) -> None:
        ...
        backend_optimizer = self._backend_optimizer
        if backend_optimizer is not None and hasattr(backend_optimizer, "cleanup"):
            backend_optimizer.cleanup()
    ...
    def generate(
        ...
        **kwargs
        ...
        if self.tokenizer and not isinstance(output, str):
            return str(self.tokenizer.decode(output[0], skip_special_tokens=True))

        return cast(Union[str, torch.Tensor], output)
