...
from cortex.inference_engine import GenerationRequest, InferenceEngine
...
class ConversationView(ScrollableContainer):
    ...
    def update_last_message(self, content: str):
        ...
        if self.messages:
            ...
            last_message.message_text = content
...
class TerminalApp(App):
    ...
    async def load_default_model(self):
        ...
        if conversation_view is None or status_bar is None:
            return
        ...
        if success:
            ...
            if model_info:
                status_bar.model_name = model_info.name
                conversation_view.add_message(
                    "system",
    ...
    async def handle_message(self, message: str):
        ...
        conversation_view.add_message("user", message, datetime.now())
        self.conversation_manager.add_message(MessageRole.USER, message)

        conversation_view.add_message("assistant", "", datetime.now())
            ...
            try:
                for token in self.inference_engine.generate(request):
                    ...
                    self.call_from_thread(
                        conversation_view.update_last_message,
                        generated_text
                    )
                ...
                if self.inference_engine.current_metrics:
                    ...
                    status_bar.tokens_per_second = metrics.tokens_per_second
                    status_bar.gpu_utilization = metrics.gpu_utilization
                    status_bar.memory_gb = metrics.memory_used_gb
    ...
    def command_clear(self):
        """Clear conversation."""
        if self.conversation_view is None:
            return
    ...
    def command_help(self):
        ...
        conversation_view.add_message("system", help_text, datetime.now())
    ...
    def command_gpu_status(self):
        ...
        conversation_view.add_message("system", status_text, datetime.now())
    ...
    async def command_benchmark(self):
        ...
        if metrics:
            ...
            conversation_view.add_message("system", benchmark_text, datetime.now())
