...
mlx_load_dataset: Optional[Callable[..., Any]]
try:
    from mlx_lm import load as mlx_load_fn
    from mlx_lm.tuner.datasets import load_dataset as mlx_load_dataset_fn
    from mlx_lm.tuner.lora import LoRALinear as LoRALinearType
    from mlx_lm.tuner.trainer import TrainingArgs as TrainingArgsType
    from mlx_lm.tuner.trainer import train as mlx_train_fn

    mlx_load = mlx_load_fn
    LoRALinear = LoRALinearType
    TrainingArgs = TrainingArgsType
    mlx_train = mlx_train_fn
    mlx_load_dataset = mlx_load_dataset_fn
except ImportError:
...
class SmartConfigFactory:
    ...
    def create_smart_config(
        ...
        config = TrainingConfig(
            ...
            gradient_accumulation_steps=int(base_config["gradient_accumulation_steps"]),

            # LoRA parameters
            lora_r=int(base_config["lora_r"]),
            lora_alpha=int(base_config["lora_alpha"]),
            ...
            max_sequence_length=int(base_config.get("max_sequence_length", 2048)),
    ...
    def generate_guidance_message(cls, config: TrainingConfig, model_name: str) -> str:
        ...
        if config.epochs >= 5:
            ...
            messages.append("Single epoch training - suitable for large datasets")
    ...
    def _apply_memory_guards(cls, cfg: Dict[str, Any], total_mem_gb: Optional[float]) -> bool:
        ...
        def effective_tokens(candidate: Dict[str, Any]) -> int:
            return int(
                candidate["batch_size"]
                * candidate["gradient_accumulation_steps"]
                * candidate.get("max_sequence_length", 2048)
            )
...
class LoRATrainer:
    ...
    def _load_base_model(self, model_name: str) -> Tuple[Optional[Any], Optional[Any]]:
        ...
        try:
            ...
            current_model_key = self.model_manager.current_model
            model = self.model_manager.model_cache.get(model_name) or (
                self.model_manager.model_cache.get(current_model_key) if current_model_key else None
            )
            tokenizer = self.model_manager.tokenizers.get(model_name) or (
                self.model_manager.tokenizers.get(current_model_key) if current_model_key else None
            )

            if not model or not tokenizer:
                logger.error("Model or tokenizer still not available after reload")
                return None, None
    ...
    def _setup_optimizer(self, model: Any, config: TrainingConfig) -> Any:
        ...
        if hasattr(model, "parameters"):
            ...
            optimizer.init({str(i): p for i, p in enumerate(trainable_params)})
